...
function Complain() {
    ...
    this.loadComplain = function (res, comp_id) {
        connection.acquire(function (err, con) {
            ...
            +` c.lat as lat, c.lng as lng, DATE_FORMAT(c.date,'%b %d %Y %h:%i %p') as date, u.name as user from complains c left outer join complain_images i on  c.id = i.complain_id join pollution_type p join user_details u where c.id = ? `
