...
function Complain() {
    ...
    this.loadComplainsChunk = function (res, user_id, start, end) {
        connection.acquire(function (err, con) {
            ...
            if(user_id !== 0){
                con.query(`select c.id as id, p.type as type ,c.res_person as res_person,c.anonymous as anonymous,SUBSTRING(c.details,1,42) as details, `+
                `DATE_FORMAT(c.date,'%b %d %Y') as date, u.name as user , IF(i.image IS NULL, FALSE, TRUE) as image, u.id as user_id, (select count(*) from comments co where co.complain_id = c.id group by complain_id) as comments, `+
                `c.expert_replied as expertReplied, c.user_replied as userReplied, c.closed as closed, `+
                ` f.is_favorite as fav from complains c join pollution_type p join user_details u left outer join complain_images i on c.id = i.complain_id and i.selected = 1 left outer join complains_favorite f on c.id = f.complain_id and f.user_id = ? where p.id = c.type and c.user_id = u.id order by c.assigned_to = ? desc, u.id = ? desc, c.date desc limit ?,? `, [user_id,user_id,user_id,start,end], function (err, result) {
                con.release();
                res.json(result);
                });
            }else{
    ...
    this.loadComplain = function (res, comp_id, userId) {
        connection.acquire(function (err, con) {
            ...
            + `join expected_action e left outer join complains_favorite f on c.id = f.complain_id and f.user_id = ? and f.is_favorite = 1 join user_details pu where c.id = ? `
            + ` and p.id = c.type and e.id = c.action and c.user_id = u.id and pu.id = c.assigned_to `, [userId,comp_id] ,function (err, result) {
                con.release();
                res.json(result);
            });
        });
...
/*    this.updateComplain = function(res, details){
                ...
                con.query('update complains set type=?, res_person=?, details=?, location=? , lat=?, lng=?, action=?, anonymous=? where id = ? ', [details.complain.pid,details.complain.person,details.complain.details, details.complain.location ,details.complain.lat, details.complain.lng, details.complain.aid, details.complain.anonymous ,details.complain.id], function(err, result){
                    ...
                    con.query('delete from complain_images where complain_id= ?', details.complain.id ,function(err,result){
                        ...
                        Async.eachOfSeries(details.images, function itOvEl(element,index,callback){
                          ...
                          if(details.images && details.images.length > 0 && details.images[index]){
                          var base64Data = details.images[index].replace(/^data:image\/jpeg;base64,/, "");
    ...
    this.updateAuthority = function(res,complainId, authId){
      console.log("^^^^^^^^^UPDATING AUTHORITY^^^^^^^");
      console.log(complainId); console.log(authId);
      connection.acquire( function(err,con){
        if(err){
          con.release();
        ...
        con.beginTransaction(function(err){
          ...
          con.query('update complains set assigned_to = ? where id = ? ', [authId,complainId], function(err, result){
              if (err) {
                  con.rollback(function() {
                      con.release();
                      res.send({ status: false, message: 'Error' });
              ...
              con.commit(function(err) {
                ...
                con.release();
                res.send({ status: true, message: 'authority updated successfully' });
                console.log('success!');
              });
            });
          })
